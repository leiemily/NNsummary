...
%% noise level
load('frac2-10tp-expmix1.mat','nl_snr');
% nl_snr = unifrnd(20,22,1,Numberof_F);
snr_linear = 10.^(nl_snr/10);% 将SNR转换为线性比例

%% s setting
load('frac2-10tp-expmix1.mat','S_save1uncut','S_para','Tp','per');
step_maxvalue = 2;

%% 9tp
stretch_para = 9;
p_smooth = 0.2*[1,1,1,0.5];

% %% 8tp
% stretch_para = 8;
% p_smooth = 0.3*[1,1,1,0.5];

% %% 7tp
% stretch_para = 7;
% p_smooth = 0.4*[1,1,1,0.5];

% %% 6tp
% stretch_para = 6;
% p_smooth = 0.45*[1,1,1,0.5];

% %% 5tp
% stretch_para = 5;
% p_smooth = 0.5*[1,1,1,0.5];

% %% 4tp
% stretch_para = 4;
% p_smooth = 0.75*[1,1,1,0.5];

%%
Tp = stretch_para*Tp/10;
S_para = stretch_para*S_para/10;
load(['frac2-',num2str(stretch_para),'tp-mix1'],'t_scale','dt00');
...
%%
save(['frac2-',num2str(stretch_para),'tp-expmix1'],'nl_snr','per','t_scale','Numberof_F','Numberof_Tp','discreteT_length','dt00','t00',...
