...
trainsetName0 = ['exp2-',num2str(j),'tp-save1d-noise'];
load(fullfile(folderPath_NNdata, trainsetName0));

testsetName0 = ['exp2-',num2str(j),'tp-test1d-noise'];
...
%%
datasetName0 = ['exp2-',num2str(j),'tp-both1-noise'];
load(fullfile(folderPath_varieddata, datasetName0),'S_lin','F_lin');
simu1 = Y_surf1;
fit_Gsurfaces;

%%
datasetName1 = ['exp2-',num2str(j),'tp-trapz1'];
...
%%
datasetName2 = ['exp2-',num2str(j),'tp-mix1'];
...
N_vali_mix = length(F_mix_vali);

vali_all = [F_step_vali,F_mix_vali];
valipre_all = [Fpre_step_vali,Fpre_mix_vali];
...
error_MAEvali_mix = 1/N_vali_mix * sum(abs(F_mix_vali- Fpre_mix_vali)./abs(F_mix_vali));
error_MSEvali_mix = 1/N_vali_mix * sum((F_mix_vali- Fpre_mix_vali).^2./(F_mix_vali).^2);
error_R2vali_mix = 1- sum((F_mix_vali- Fpre_mix_vali).^2)/sum((F_mix_vali-mean(F_mix_vali)).^2);
