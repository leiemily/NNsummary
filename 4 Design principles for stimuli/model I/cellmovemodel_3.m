function P_tumble = cellmovemodel_3(Yp)
% 应用于cellmodel_3函数
a1 = 0.0174001;
b1 = 1.32887;
a2 = 12.0809;
...
% P_ccww = nchoosek(nf, omega) * (mu_f ./(mu_f + lambda_f)).^omega .* ( lambda_f ./(mu_f + lambda_f)).^(nf - omega);
% P_ccww_1 = nchoosek(nf, omega-1) * (mu_f ./(mu_f + lambda_f)).^(omega-1) .* ( lambda_f ./(mu_f + lambda_f)).^(nf - omega +1);
